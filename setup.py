...
#!/bin/bash
IP={ssh_ip}
echo "\e[32m[*] Connecting To {ssh_un}..."
sleep 0.5
echo "\e[32m[*] Generated With Auto_SSH"
if ping -q -c 1 -W 1 $IP >/dev/null; then
  echo "\e[32m[*] Connected To {ssh_un}"
  sleep 1
  clear
  ssh {ssh_un}@{ssh_ip}
else
  echo "\e[93m[*] Cant Connect To {ssh_un} No Reply!"
  echo "\e[93m[*] Exiting..."
  exit
fi """
os.system('touch ssh')
